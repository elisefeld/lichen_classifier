...
def build_lichen_classifier(input_shape: tuple,
    ...
    x = keras.layers.Lambda(MODEL_MAP[base_model_name]['preprocess'], name='preprocess')(inputs)
    x = keras.layers.Resizing(dim, dim, crop_to_aspect_ratio=True, name='resize')(x)
    x = keras.layers.CenterCrop(crop_dim, crop_dim, name='center_crop')(x) 

    x = keras.layers.RandomFlip('horizontal', name='random_flip')(x)
    x = keras.layers.RandomRotation(rotation, name='random_rotation')(x)
    x = keras.layers.RandomContrast(contrast, name='random_contrast')(x)
    x = keras.layers.RandomTranslation(translation, translation, name='random_translation')(x)
    ...
    x = keras.layers.Dense(1024, activation='selu', kernel_initializer='lecun_normal', name='dense_1024')(x)
    x = keras.layers.BatchNormalization(name='batch_norm_1')(x)
    x = keras.layers.Dropout(0.3, name='dropout_1')(x)

    x = keras.layers.Dense(512, activation='selu', kernel_initializer='lecun_normal', name='dense_512')(x)
    x = keras.layers.BatchNormalization(name='batch_norm_2')(x)
    x = keras.layers.Dropout(0.2, name='dropout_2')(x)

    x = keras.layers.Dense(128, activation='selu', kernel_initializer='lecun_normal', name='dense_128')(x)
    x = keras.layers.BatchNormalization(name='batch_norm_3')(x)
    x = keras.layers.Dropout(0.1, name='dropout_3')(x)

    outputs = keras.layers.Dense(num_classes, activation='softmax', dtype='float32', name='output_layer')(x)
